...
type ShowWordListScreenRouteProp = RouteProp<
  RootStackPropsList,
  'ShowWordList'
>;
...
const ShowWordListScreen: React.FC<ShowWordListScreenProps> = ({
  navigation,
  route,
}) => {
  ...
  const renderOrderJapanese = () => {
    if (lineIndex === wordList.length) {
      ...
      return (
        <Text style={{ ...styles.selectedLetter, color: 'red' }}>
          よくできました
        </Text>
      );
    }
    ...
    if (wordList[lineIndex].length === 1) {
      return (
        <TouchableOpacity
          key={0}
          onPress={() => handleLetterPress(wordList[lineIndex], 0)}
        >
          <Text
            style={{
              ...styles.selectedLetter,
              color: selectedLetters[`${wordList[lineIndex]}0`]
                ? 'red'
                : 'black',
            }}
          >
  ...
  const handleLetterPress = (letter: string, index: number) => {
    if (
      index === 0 ||
      selectedLetters[`${showingWords[index - 1]}${index - 1}`] === true
    ) {
      ...
      if (
        showingWords.length === 1 ||
        (index !== 0 && showingWords.length === index + 1)
      ) {
        setIsOverLine(true);
      }
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.showWord}>{renderOrderJapanese()}</View>
      <View style={{ ...styles.buttonContainer, width: '90%' }}>
        ...
        <RectButton
          style={
            isOverLine
              ? { ...styles.button, backgroundColor: '#73fa73' }
              : { ...styles.button, backgroundColor: '#D3D3D3' }
          }
          enabled={isOverLine}
          onPress={initNextLine}
        >
          <Text
            style={
              isOverLine
