...
const db = SQLite.openDatabase(
  { name: 'RecordDB.db', location: 'default' },
  () => {},
  (error) => {
    console.log(error);
  }
...
const RecordVoiceScreen: React.FC = () => {
  ...
  const saveRecording = (filePath: string) => {
    db.transaction((tx) => {
      tx.executeSql(
        'CREATE TABLE IF NOT EXISTS Recordings (id INTEGER PRIMARY KEY NOT NULL, PATH  TEXT NOT NULL)'
      );
      tx.executeSql(
        'INSERT INTO Recordings (path) VALUES (?)',
        [filePath],
        () => {
