...
const Items: React.FC<ItemsProps> = ({ items, onPressHandler }) => {
  ...
  return (
    <FlatList
      data={items}
      renderItem={({ item }) => (
        <Item item={item} onPressHandler={onPressHandler} />
      )}
...
const MakeListsWordsScreen: React.FC<Props> = ({ navigation, route }) => {
    ...
    db.transaction((tx) => {
      tx.executeSql(
        ...
        (_, { rows }) => {
          if (rows.length === 0) {
            //
            tx.executeSql('insert into words (listId, word) values (?, ?)', [
              id,
              'おはよう',
            ]);
            tx.executeSql('insert into words (listId, word) values (?, ?)', [
  ...
  const add = (text: string | null) => {
    ...
    db.transaction(
      (tx) => {
        tx.executeSql(
          'insert into words (listId, word) values (?, ?)',
          [route.params.listId, text],
          (_, { insertId }) => {
